...
public class AuthInterceptor implements HandlerInterceptor {
    ...
    /**
     * preHandle 抛出异常或者返回 false 这个就不会执行
     * 但是只要返回 true 进入下一步就算报错也不会影响执行
     * 所以需要在 HttpRequestWrapperFilter 套一层 try finally 防止内存泄露
     */
    @Override
    ...
    public List<String> getPermissionList(Object loginId, SpaceUserAuthContext authContext) {
        ...
        if (spaceUserId != null) {
            ...
            SpaceUser loginSpaceUser = spaceUserService.getBySpaceUserId(spaceUser.getSpaceId(), userId);
        ...
        if (space.getSpaceType() == SpaceTypeEnum.PRIVATE.getValue()) {
            ...
            spaceUser = spaceUserService.getBySpaceUserId(spaceId, userId);
            if (spaceUser == null) {
                return new ArrayList<>();
            }
            return spaceUserAuthManager.getPermissionsByRole(spaceUser.getSpaceRole());
